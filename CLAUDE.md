...
- **UI 0→1 revamp — WAVE 3: design-system adoption across 16 secondary screens (2026-06-23; web-only, COMMITTED + PUSHED `cf1a9f7` on branch `p0-6-tenant-page-engine`, NOT merged to main / NOT deployed; web `tsc` clean incl. isolated-from-other-lanes + `eslint` 0/0 + 6-screen screenshot-verified).** Ran as 4 parallel agents over disjoint web file sets (same guardrails: no foundation/`ui/` edits, no other-lane files). Visual-only — every arbitrary `text-[Npx]` → type ramp (count 0 across all 16), hand-rolled status pills → `<Badge>` semantic variants, hand-rolled buttons/spinners → `<Button variant loading>`, empty/error branches → `<EmptyState>`/`<ErrorState>`, raw-hex chrome → CSS vars, focus rings standardized. Lanes: **Page-Engine core** (`pages/pages-view` 73→0 the paid-core list/editor, `pages/keyword-review`, `app/(app)/dashboard`), **drawers+research** (`leads/lead-detail-drawer`, `research/opportunities-explorer`=/research, `opportunities/prospect-edit-drawer`), **brand+theme** (`theme/theme-settings-view` — customer dynamic-theme swatches/preview kept as DATA, the explicit exception; `brand/brand-library`, `brand/brand-memory-editor`, `competitors/competitor-analysis-view`), **secondary workspaces** (`ai-search`, `conversion-audit`, `content`, `solutions`, `alerts`, `search-results`). Verified all of /pages /research /theme /solutions /alerts /ai-search → HTTP 200 + screenshot-clean. **Deliberately NOT touched (other lanes own them):** `billing/*`, `admin/*`, `shell/command-palette`, `shell/topbar`, `system/app-feedback`, `nav-config`. ⚠️ Commit landed on top of the image-gen lane's `cc5d64f` (their `image-gen.service`+`page-engine.service`) — multi-account interleave worked, each lane committed only its own files. **REMAINING toward full adoption:** the contested shell/billing/admin web screens (other lanes), the 3 auth + 5 system screens, public-feed polish; and the shared `ui/table` component still isn't universally imported (screens adopted the treatment, not the component).
- **UI 0→1 revamp — WAVE 2: per-screen adoption sweep (2026-06-23; web-only, UNCOMMITTED on disk alongside other lanes' WIP, web `tsc --noEmit` clean + `eslint` 0/0 + 6-screen screenshot-verified [Home/Authority/Leads/Settings/Analytics/Brand], NOT deployed). Ran as 4 parallel agents over strictly disjoint file sets, NOT touching the Wave 1 foundation (globals.css `@theme`, ui/ primitives, Home, page-header, panel).** Lanes: **(1) data-tables** — `opportunities-view.tsx` (+355), `leads/leads-view.tsx` (+246, the worst offender: 820px min-width table + `hidden sm:flex` filters), `performance/tracked-pages-table.tsx` adopt the house table treatment + semantic `Badge` variants + mobile card fallback. **(2) forms + settings** — `settings/settings-view.tsx` (+109), `pages/publishing-settings.tsx`, `leads/lead-config-actions.tsx`, `onboarding-wizard.tsx`: raw inputs → `Field`/label-association, raw `<input type=checkbox>` toggles → NEW **`ui/switch.tsx`** primitive (role=switch, a11y, busy spinner). **(3) analytics + charts** — `analytics/analytics-workspace.tsx` (+124, removed the `🎉` emoji, honest chart empty-states), `performance/{rank,traffic}-chart.tsx`, `sparkline`, `progress-bar`. **(4) brand + solution** — `brand/{brand-kit,brand-workspace}.tsx`, `competitors/competitor-analysis-view.tsx`, `dashboard/{brand-memory-card,brand-scorecard,action-center,growth-plan}.tsx`. **Integrity fixes on pickup (the agents were interrupted mid-edit):** (a) **real build-breaking bug** — `lead-config-actions.tsx` referenced `minScoreId` (htmlFor/id for label assoc) but never declared it → `tsc` TS2552 ×2; added `const minScoreId = useId()`. (b) **completed a half-done swap** — same file imported `Switch` but left the hand-rolled `<button role=switch>` in place (unused-import lint warning); finished the conversion to `<Switch checked … onCheckedChange … />`. Verify gotcha from Wave 1 still applies (`rm -rf .next` after the `@theme` edit; boot web with blank Clerk keys to dodge the dev-key redirect loop). **REMAINING toward full adoption:** the `ui/table` migration is partial (lanes adopted the treatment but the shared `ui/table` component isn't universally imported yet); 3 auth + 5 system screens not in this pass; no commit/deploy yet.
- **UI 0→1 revamp — WAVE 1: foundation + flagship + feed (2026-06-22; web-only, UNCOMMITTED on disk, web tsc clean + screenshot-verified; NOT deployed). Direction = "Refined Aurora" (operator-chosen): elevate the existing light/violet/aurora identity to best-in-class, foundation-first.** Three parallel audits found the root cause: a real token system + full primitive lib exist but screens BYPASS them (ui/table, ui/input, ui/badge each imported by ZERO files; 783 arbitrary `text-[Npx]`; muted-fg ~3:1 fails WCAG). **Shipped this wave (lifts every screen at once):** (1) **`globals.css` token layer** — landed the 7-step type ramp in `@theme` (`--text-display/kpi/title/h-card/body/label/micro`, each with line-height/tracking/weight → use `text-kpi` etc., not `text-[28px]`); darkened `--muted-foreground` `#8a909a`→`#696f7a` light / `#80868f`→`#9aa0aa` dark (clears AA); added elevation tiers `--shadow-xs/raised` + `.font-heading`/`.ease-expo` + `--ease-out-expo`/`--ease-spring`. (2) **Converged primitives:** `ui/card.tsx` ring→house `border-border shadow-card`; `ui/button.tsx` +`brand` variant +`loading` prop (Loader2+aria-busy); `ui/badge.tsx` + semantic `brand/positive/negative/warning/info/muted` variants (collapses 51 hand-rolled pills); NEW `ui/empty-state.tsx` + `ui/error-state.tsx`. (3) **Flagship Home** (`(app)/page.tsx`) re-ranked: KPI hero row leads, Momentum deduped to a slim caption (was a full band duplicated in analytics), type ramp applied. (4) **`page-header.tsx`** (all 22 routes): `text-display` title + brand-dot eyebrow + aurora bottom-fade. (5) **Public feed cleanup** (`feeds/[slug]`+`feeds/page.tsx`): removed "Powered by Puter", the debug "Canonical:" line, and the internal page-type badge. (6) **Hardcoded sweep** ("remove all hardcoded"): ProgressBar default `to`→`var(--info)` + dropped `from/to="#6C4CF1/#2D6BFF"` callers; rank/traffic chart hex stops→`var(--chart-*)`/`--brand` (now follow dark mode); onboarding `#0E1116`→`bg-foreground/text-background`; stale `Northwind Labs` workspace fallback→`Your Workspace`. (7) **Real bug:** lead-detail-drawer `Promise.all` had no `.catch`→infinite spinner; added `.catch`+`.finally(setLoaded)`. ⚠️ **VERIFY GOTCHA: after editing `@theme` in globals.css you MUST `rm -rf .next`** — Turbopack caches `@theme` and HMR serves stale tokens (cost me 3 reboots; confirmed via served-CSS grep). Local verify booted web with **blank Clerk keys** (`NEXT_PUBLIC_CLERK_PUBLISHABLE_KEY= CLERK_SECRET_KEY=`) to dodge a dev-key "infinite redirect loop" on `/`. **REMAINING (per-screen adoption sweep, ~30 screens — WAVE 2+):** route the 6 hand-rolled tables→`ui/table` (+ mobile card fallback; Leads table worst: 820px min-width, filters `hidden sm:flex`), 22 raw inputs→`ui/input`+`Field` (label association/a11y), 51 pills→`Badge` variants, focus rings via primitives, settings raw `<input type=checkbox>`→switch, dedupe momentum/score-bar treatments, chart empty-states + emoji removal (analytics-workspace 🎉). Full screen inventory = 51 routes/views (39 app + 4 public + 3 auth + 5 system).
