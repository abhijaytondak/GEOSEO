        ...
        <AreaChart data={series} margin={{ top: 10, right: 10, bottom: 0, left: -8 }}>
          <defs>
            ...
            <filter id="rank-glow" x="-20%" y="-40%" width="140%" height="180%">
              <feDropShadow dx="0" dy="4" stdDeviation="6" floodColor="var(--brand)" floodOpacity="0.35" />
            </filter>
          </defs>
          ...
          <Area
            ...
            dot={(p) => (
              <LeadingDot key={`rank-${p.index}`} cx={p.cx} cy={p.cy} index={p.index} last={last} color="var(--brand)" />
            )}
