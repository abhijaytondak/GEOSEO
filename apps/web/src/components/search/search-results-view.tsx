  ...
  return (
    <div className="grid grid-cols-1 gap-5 lg:grid-cols-[220px_1fr]">
      ...
      <aside className="lg:sticky lg:top-4 lg:self-start">
        <div className="mb-2 text-micro font-semibold uppercase text-muted-foreground">
          Filter by type
        </div>
        <div className="flex flex-wrap gap-1.5 lg:flex-col lg:items-stretch">
          <button
            ...
            className={cn(
              "rounded-lg px-2.5 py-1.5 text-left text-label font-medium transition-colors focus-visible:outline-none focus-visible:ring-3 focus-visible:ring-ring/50",
          ...
          {facets.map((f) => (
            <button
              ...
              className={cn(
                "flex items-center justify-between gap-2 rounded-lg px-2.5 py-1.5 text-left text-label font-medium transition-colors focus-visible:outline-none focus-visible:ring-3 focus-visible:ring-ring/50",
      ...
      <div className="min-w-0">
        ...
        {results.length === 0 ? (
          <EmptyState
            tone="prompt"
            icon={Search}
            title={`No results${query.trim() ? ` for “${query.trim()}”` : ""}`}
            description={
              <>
          ...
          <div className="space-y-5">
            {grouped.map((group) => (
              <section key={group.type}>
                <h2 className="mb-2 text-micro font-semibold uppercase text-muted-foreground">
                  ...
                  {group.items.map((res) => {
                    ...
                    return (
                      <li key={res.id}>
                        <button
                          onClick={() => open(res)}
                          className="flex w-full items-center gap-3 rounded-xl border border-border bg-card p-3.5 text-left transition-colors hover:border-brand/40 hover:bg-surface-sunken focus-visible:outline-none focus-visible:ring-3 focus-visible:ring-ring/50"
                        >
                          ...
                          <span className="min-w-0 flex-1">
                            <span className="block truncate text-label font-semibold text-foreground">{res.title}</span>
                            {res.subtitle && (
                              <span className="block truncate text-label text-muted-foreground">{res.subtitle}</span>
                            )}
                          </span>
                          {res.metrics?.slice(0, 2).map((m) => (
                            <span key={m.label || String(m.value)} className="hidden shrink-0 text-right sm:block">
                              {m.label && <span className="block text-micro uppercase text-muted-foreground">{m.label}</span>}
                              <span className="tnum text-label font-semibold text-foreground">{m.value}</span>
                            </span>
                          ))}
                          {res.status && (
