...
import { Save, RefreshCw, Tag, Sparkles, Check, Mail, Clock, type LucideIcon } from "lucide-react";
...
const fieldClass =
  "w-full rounded-lg border border-border bg-surface-sunken px-3 py-2 text-sm outline-none transition-colors focus:border-ring focus:bg-card";
  ...
  return (
    <>
      ...
      <div className="flex min-h-0 flex-1 flex-col gap-5 overflow-y-auto px-6 py-5">
        ...
        <div className="rounded-xl border border-border bg-surface-sunken p-3.5">
          <div className="flex items-center justify-between">
            ...
            <span className="tnum text-[18px] font-bold text-brand">{prospect.impactScore}</span>
          </div>
          ...
          <p className="mt-3 text-[12px] leading-relaxed text-muted-foreground">{prospect.rationale}</p>
        </div>
        ...
        {/* contact email */}
        <div>
          <label htmlFor="prospect-email" className="text-[11px] font-semibold uppercase tracking-[0.06em] text-muted-foreground">
            Contact email
          </label>
          <input
            id="prospect-email"
        ...
        {/* tags */}
        <div>
          <label htmlFor="prospect-tags" className="flex items-center gap-1.5 text-[11px] font-semibold uppercase tracking-[0.06em] text-muted-foreground">
          ...
          <input
            ...
            className={cn(fieldClass, "mt-2 h-10")}
          />
          <p className="mt-1.5 text-[11.5px] text-muted-foreground">Comma-separated.</p>
        </div>

        {/* notes */}
        <div>
          <label htmlFor="prospect-notes" className="text-[11px] font-semibold uppercase tracking-[0.06em] text-muted-foreground">
            Notes
          </label>
          <textarea
            id="prospect-notes"
        ...
        <div>
          <span className="text-[11px] font-semibold uppercase tracking-[0.06em] text-muted-foreground">Activity</span>
          <ul className="mt-2 space-y-2">
            {activity.map((ev) => (
              <li key={ev.label} className="flex items-center gap-2.5 text-[12.5px]">
                ...
                {ev.when && <span className="text-[11px] text-muted-foreground">{ev.when}</span>}
              </li>
            ))}
          </ul>
        </div>
      ...
      <div className="flex items-center justify-end gap-2 border-t border-border px-6 py-4">
        ...
        <Button className="h-9 rounded-full px-4" onClick={save} disabled={saving}>
